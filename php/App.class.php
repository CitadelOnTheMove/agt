...
/**
 * The class to hadle the creation and loading of the applications
...
class App {
     ...
     */
    public function __construct($uid, $name, $description, $userId, $datasetIds, $cityIds, $color, $darkColor) {
        $this->uid = $uid;
        $this->name = $name;
        ...
        try {
            ...
            if ($result = $sth->fetch(PDO::FETCH_ASSOC)) {
                ...
                try {
                    ...
                    return new App($uid, $name, $description, $userId, $datasetIds, $cityIds, $color, $darkColor);
                } catch (Exception $e) {
                    if (DEBUG)
                        $sth->debugDumpParams();
    ...
    public static function createFromArray($assocArray, $userId) {
        if (!empty($assocArray))
            return new App(null, $assocArray['name'], $assocArray['description'], $userId, $assocArray['datasetIds'], $assocArray['cityIds'], $assocArray['color'], $assocArray['darkColor']);
        return false;
    }

    // CityIds[], AppName, AppDescription, DatasetIds[], ColorId. 
    public function save() {

        $this->uid = trim(App::getGUID(), '{}');
        $sql = "INSERT INTO apps VALUES(:uid, :name, :userId, Now(), :description, :isDeleted)";
        $sqlParams = array(':uid' => $this->uid, ':name' => $this->name, ':userId' => $this->userId, ':description' => $this->description,':isDeleted' => 0);
